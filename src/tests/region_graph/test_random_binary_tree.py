...
def test_random_binary_tree(
    num_variables: int, depth: int, num_repetitions: int
) -> None:
    ...
    if depth < 0:
        assert (len(rg.topological_layers(bottom_up=False)) - 1) == int(
            math.ceil(math.log2(num_variables))
        )
    else:
