...
parser.add_argument(
    "tboard_path",
    default="tboard-runs/gpt2-commongen-grid",
    type=str,
    help="The Tensorboard runs path",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to plt min, median and max areas",
)
...
if __name__ == "__main__":
    ...
    for k, model_name in enumerate(model_names):
        ...
        if model_name == "BornPC":
            rows_to_keep = {
                "init_method": ["uniform", "positive-skewed-normal", "normal"],
                "learning_rate": [5e-3, 1e-2, 5e-2],
            }
            model_df = model_df[model_df["init_scale"] == 0.1]
        else:
            rows_to_keep = {
                "init_method": ["uniform", "dirichlet", "log-normal"],
                "learning_rate": [5e-3, 1e-2, 5e-2],
            }
        ...
        if args.median:
            label = f"{format_model(model_name)}" if should_label else None
            median_metrics = sorted(
                list(
                    map(lambda x: (x[0], np.median(x[1])), metrics[model_name].items())
                ),
                key=lambda x: x[0],
            )
            bot_metrics = sorted(
                list(
                    map(
                        lambda x: (x[0], np.quantile(x[1], q=0.05)),
                        metrics[model_name].items(),
                    )
                ),
                key=lambda x: x[0],
            )
            top_metrics = sorted(
                list(
                    map(
                        lambda x: (x[0], np.quantile(x[1], q=0.95)),
                        metrics[model_name].items(),
                    )
                ),
                key=lambda x: x[0],
            ...
            for k in sorted(metrics[model_a].keys() & metrics[model_b].keys()):
                ...
                if ts == "mannwithneyu":
                    s, p = stats.mannwhitneyu(
                        lls_b, lls_a, method="exact", alternative=al
                    )
                elif ts == "ttest":
    ...
    ax.annotate(
        f"{format_metric(args.metric)}",
        xy=(0, 1.05),
        xytext=(-1.5 * rcParams["ytick.major.pad"], 1),
        ha="right",
        va="bottom",
        xycoords="axes fraction",
        textcoords="offset points",
    ...
    plt.savefig(
        os.path.join(
            "figures",
            "gpt2dist",
            f'multi-lines-{args.metric}-{"train" if args.train else "test"}.pdf',
        )
    )
