...
if __name__ == "__main__":
    ...
    truth_pdf = artificial_kde(
        args.dataset, bandwidth=dataset_kde_bandwidths[args.dataset]
    )
    ...
    plt.savefig(
        os.path.join("figures", "artificial-data", f"{args.dataset}-ground-truth.png"),
        dpi=1200,
    )
    ...
    for m, eal, eid, pdf in zip(models, exp_alises, exp_ids, pdfs):
        ...
        plt.savefig(
            os.path.join("figures", "artificial-data", f"{args.dataset}-{m}-{eal}.png"),
            dpi=1200,
        )
