...
parser.add_argument(
    ...
    required=True,
    type=int,
    help="The number of bins for discretization",
)
parser.add_argument(
...
def artificial_pmf(dataset: str, discretize_bins: int) -> np.ndarray:
    ...
    return discrete_samples_hmap(
        data, xlim=(0, discretize_bins - 1), ylim=(0, discretize_bins - 1)
    )


def load_pmf(
    model: str, dataset: str, exp_alias: str, exp_id: str, binomials: bool = False
) -> np.ndarray:
    suffix = "binomials" if binomials else "categoricals"
    filepath = os.path.join(
        args.checkpoint_path,
        f"artificial-discrete-{suffix}",
        dataset,
        model,
        exp_alias,
        exp_id,
...
if __name__ == "__main__":
    ...
    models = ["MonotonicPC", "BornPC", "BornPC"]

    exp_alises = ["", "monotonic", "non-monotonic"]
    ...
    dataset_num_components = {
        ...
        "binomials": {"mring": 96, "banana": 32, "funnel": 64, "cosine": 64},
    }
    ...
    pmfs_categoricals = [
        load_pmf(
            m,
            args.dataset,
            f"{eal}-b{args.discretize_bins}" if eal else f"b{args.discretize_bins}",
            eid.format(dataset_num_components["categoricals"][args.dataset]),
            binomials=False,
        )
    ...
    pmfs_binomials = [
        load_pmf(
            ...
            args.dataset,
            f"{eal}-b{args.discretize_bins}" if eal else f"b{args.discretize_bins}",
            eid.format(dataset_num_components["binomials"][args.dataset]),
            binomials=True,
        )
        for m, eal, eid in zip(models, exp_alises, exp_ids)
    ]
    vmax = np.maximum(
        np.maximum(np.max(truth_pmf), np.max(pmfs_categoricals)), np.max(pmfs_binomials)
    )
    vmin = 0.0
    ...
    plt.savefig(
        os.path.join(
            "figures", "artificial-data", f"{args.dataset}-ground-truth-pmf.png"
        ),
        dpi=1200,
    )

    for idx, (m, eal, eid, pmf) in enumerate(
        zip(models, exp_alises, exp_ids, pmfs_categoricals)
    ):
        num_rows, num_cols = 1, 1
        setup_tueplots(num_rows, num_cols, rel_width=0.2, hw_ratio=1.0)
        fig, ax = plt.subplots(num_rows, num_cols)
        ...
        plt.savefig(
            os.path.join(
                "figures",
                "artificial-data",
                f"{args.dataset}-{m}-{eal}-categoricals.png",
            ),
            dpi=1200,
        )

    for idx, (m, eal, eid, pmf) in enumerate(
        zip(models, exp_alises, exp_ids, pmfs_binomials)
    ):
        num_rows, num_cols = 1, 1
        setup_tueplots(num_rows, num_cols, rel_width=0.2, hw_ratio=1.0)
        fig, ax = plt.subplots(num_rows, num_cols)
        ...
        plt.savefig(
            os.path.join(
                "figures", "artificial-data", f"{args.dataset}-{m}-{eal}-binomials.png"
            ),
            dpi=1200,
        )
