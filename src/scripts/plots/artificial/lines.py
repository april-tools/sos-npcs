...
parser.add_argument(
    ...
    default="",
    type=str,
    help="The processed CSV file with mean and standard deviations of metrics",
)
...
if __name__ == "__main__":
    ...
    for i, dataset in enumerate(datasets):
        ...
        for k, (model_name, exp_alias) in enumerate(
            ...
            model_df = df_dataset[
                (df_dataset["model"] == model_name)
                & (df_dataset["exp_reparam"] == exp_reparam)
            ]
    ...
    if args.label:
        plt.savefig(
            os.path.join(
                "figures",
                "artificial-data",
                f"multi-lines-{args.metric}-{args.label}.pdf",
            )
        )
    else:
        plt.savefig(
            os.path.join("figures", "artificial-data", f"multi-lines-{args.metric}.pdf")
        )
