...
parser.add_argument(
    "path", default="checkpoints", type=str, help="The checkpoints path"
)
parser.add_argument(
    "--show-ellipses",
    default=False,
...
parser.add_argument(
    "--title", default=False, action="store_true", help="Whether to show a title"
)
parser.add_argument(
    "--alt-title",
    default=False,
...
parser.add_argument(
    "--dpi", type=int, default=192, help="The DPI for PNG rasterization"
)
parser.add_argument(
    "--prune",
    default=False,
...
def format_model_name(m: str, num_components: int, alt: bool = False) -> str:
    if alt:
        ...
        if m == "MonotonicPC":
            return rf"GMM ($K \!\! = \!\! {num_components}$)"
        elif m == "BornPC":
            return rf"SGMM ($K \!\! = \!\! {num_components}$)"
    return m
...
def load_mixture(
    ...
    model: TensorizedPC = setup_model(
        model_name,
        metadata,
        num_components=num_components,
        complex="complex" in exp_id_fmt,
    )
...
def plot_mixture_ellipses(
    mixture: TensorizedPC, ax: plt.Axes, max_num_components: Optional[int] = None
):
...
if __name__ == "__main__":
    ...
    models = ["MonotonicPC", "MonotonicPC", "BornPC", "BornPC"]
    ...
    truth_pdf = np.load(
        os.path.join(
            ...
            "ring",
            models[0],
            exp_id_formats[0].format(num_components[0], learning_rates[0], 64),
            "gt.npy",
        )
    )
    # truth_pdf = ring_kde()
    ...
    mixtures = [
        ...
        for m, eif, nc, lr in zip(
            models, exp_id_formats, num_components, learning_rates
        )
    ]
    ...
    data_pdfs = [(None, truth_pdf, "Ground Truth", -1)] + list(
        zip(mixtures, pdfs, models, num_components)
    )
    for idx, (p, pdf, m, nc) in enumerate(data_pdfs):
        ...
        for j, ps in enumerate(plot_settings):
            ...
            if args.title:
                title = (
                    f"{format_model_name(m, nc, alt=args.alt_title)}"
                    if p is not None
                    else m
                )
            else:
            ...
            if args.prog_ellipses and p is not None:
                filename = (
                    f"pdfs-ellipses-{idx}-{j}.png"
                    if args.show_ellipses
                    else f"pdfs-{idx}-{j}.png"
                )
                subdir = "progressive"
            else:
                filename = (
                    f"pdfs-ellipses-{idx}.png"
                    if args.show_ellipses
                    else f"pdfs-{idx}.png"
                )
                subdir = "plain"
            ...
            plt.savefig(
                os.path.join("figures", "gaussian-ring", subdir, filename), dpi=args.dpi
            )
            plt.close()
