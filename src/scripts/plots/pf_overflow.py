...
if __name__ == "__main__":
    ...
    for dtype in [torch.float32, torch.float64]:
        ...
        results[dtype_name] = {"vars": nvs, "pfs": pfs, "log_pfs": log_pfs}
    ...
    ax.plot(
        results["fp32"]["vars"][:fp32_idx],
        results["fp32"]["pfs"][:fp32_idx],
        linestyle="dotted",
        c="C0",
        marker="o",
        markersize=6,
    ...
    ax.hlines(
        y=fp64_max,
        xmin=results["fp32"]["vars"][fp32_idx],
        xmax=results["fp64"]["vars"][fp64_idx - 1],
        colors="C1",
        ls="dotted",
        lw=2,
        alpha=0.7,
    ...
    ax.hlines(
        y=fp64_max,
        xmin=results["fp64"]["vars"][fp64_idx - 1],
        xmax=results["fp64"]["vars"][fp64_idx],
        colors="C3",
        ls="dotted",
        lw=2,
        alpha=0.9,
    ...
    ax.plot(
        results["fp64"]["vars"][fp32_idx - 1 : fp64_idx],
        results["fp64"]["pfs"][fp32_idx - 1 : fp64_idx],
        linestyle="solid",
        c="C0",
        marker="o",
        markersize=6,
    ...
    ax.scatter(
        results["fp32"]["vars"][fp32_idx:fp64_idx],
        np.full(fp64_idx - fp32_idx, fp64_max),
        marker="X",
        s=150,
        c="C1",
        label=r"fp32 $+\infty$",
    ...
    ax.scatter(
        results["fp64"]["vars"][fp64_idx:],
        np.full(len(results["fp64"]["vars"]) - fp64_idx, fp64_max),
        marker="X",
        s=150,
        c="C3",
        label=r"fp64 $+\infty$",
