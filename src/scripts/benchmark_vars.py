...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="The evaluation datasets, separated by space",
)
parser.add_argument(
    "--num-iterations",
    type=int,
...
def entry_uniform_hparams_configuration() -> Tuple[dict, list]:
    ...
    for dataset in datasets:
        ...
        metadata, (train_dataloader, valid_dataloader, test_dataloader) = (
            setup_data_loaders(dataset, "datasets", batch_size=1)
        )
...
if __name__ == "__main__":
        ...
        hparams_conf = list(
            map(
                lambda hps: list(
                    map(
                        lambda h: dict(
                            map(lambda x: tuple(x.split("=")), h.split(" "))
                        ),
                        hps,
                    )
                ),
    ...
    for idx, (m, br) in enumerate(bench_results.items()):
        ...
        sc_c = ax[0].scatter(
            ...
            color=f"C{idx}",
            alpha=0.5,
            s=list(map(lambda t: _bubble_size(t[1]), eval_c_results)),
            marker="o",
            label=desc_c,
        )
        ax[0].scatter(
            num_variables, eval_c_results_time, color="k", alpha=0.6, s=1, marker="o"
        )
        if args.specific_hparams:
        ...
        sc_Z = ax[1].scatter(
            ...
            color=f"C{idx}",
            alpha=0.5,
            s=list(map(lambda t: _bubble_size(t[1]), eval_Z_results)),
            marker="o",
            label=desc_Z,
        )
        ax[1].scatter(
            num_variables, eval_Z_results_time, color="k", alpha=0.6, s=1, marker="o"
        )
        if args.specific_hparams:
    ...
    ax[0].annotate(
        r"$|\mathbf{X}|$",
        xy=(1, 0),
        xytext=(1, -1 * rcParams["xtick.major.pad"]),
        ha="right",
        va="top",
    ...
    ax[0].legend(
        ...
        labelspacing=1.4 if args.specific_hparams else 2.2,
        frameon=False,
